...
pub fn output_block(state: &mut ZVMState, addr: u32) -> String {
    ...
    let output_instruction = |inst: &Instruction| -> String {
        ...
        let mut code = match inst.opcode {
            ...
            /* set_colour */ 27 => runtime("io.set_colour", operands),
            ...
            /* print_addr */ 135 => format!("e.io.print(2,{})", op_a),
            /* remove_obj */ 137 => runtime("remove_obj", operands),
            /* print_obj */ 138 => format!("e.io.print(3,{})", op_a),
            /* ret */ 139 => format!("return {}", op_a),
            /* jump */ 140 => format!("e.pc={}+{}", op_a, inst.next - 2),
            /* print_paddr */ 141 => format!("e.io.print(2,{}*{})", op_a, addr_multiplier),
            ...
            /* print */ 178 => format!("e.io.print(2,{})", inst.text.unwrap()),
            /* print_ret */ 179 => format!("e.io.print(2,{});e.io.print(1,13);return 1", inst.text.unwrap()),
            /* nop */ 180 => String::new(),
            /* save */ 181 => save(inst),
            ...
            /* new_line */ 187 => String::from("e.io.print(1,13)"),
            /* show_status */ 188 => if version < 4 { String::from("e.io.v3_status()") } else { String::new() },
            ...
            /* read */ 228 => format!("e.io.read({},{})", if version < 5 { 0 } else { inst.result.unwrap() }, args(operands)),
            /* print_char */ 229 => format!("e.io.print(4,{})", op_a),
            /* print_num */ 230 => format!("e.io.print(0,{})", op_a),
            /* random */ 231 => runtime("random", operands),
            /* push */ 232 => format!("t={};s[e.sp++]=t", op_a),
            /* pull */ 233 => format!("e.indirect({},s[--e.sp])", op_a),
            /* split_window */ 234 => runtime("io.split_window", operands),
            /* set_window */ 235 => runtime("io.set_window", operands),
            /* erase_window */ 237 => runtime("io.erase_window", operands),
            /* erase_line */ 238 => runtime("io.erase_line", operands),
            /* set_cursor */ 239 => format!("e.io.set_cursor({}-1,{}-1)", op_a, op_b),
            /* get_cursor */ 240 => runtime("io.get_cursor", operands),
            /* set_text_style */ 241 => runtime("io.set_text_style", operands),
            /* buffer_mode */ 242 => String::new(),
            /* output_stream */ 243 => format!("e.pc={};e.io.output_stream({})", inst.next, args(operands)),
            /* input_stream */ 244 => runtime("io.input_stream", operands),
            /* sound_effect */ 245 => String::new(),
            /* read_char */ 246 => format!("e.io.read_char({},{})", inst.result.unwrap(), if operands.len() == 0 { String::from("1") } else { args(operands) }),
